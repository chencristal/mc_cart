...
if (class_exists('MailChimp_WooCommerce_MailChimpApi')) return;
...
class MailChimp_WooCommerce_MailChimpApi
{
    ...
    public function addStoreOrder($store_id, MailChimp_WooCommerce_Order $order, $silent = true)
    {
        try {
            ...
            // update_option('mailchimp-woocommerce-resource-last-updated', time());   // chen_check
    ...
    protected function applyCurlOptions($method, $url, $params = array())
    {
        ...
        return array(
            ...
            CURLOPT_SSL_VERIFYHOST => false,        // chen_debug
            CURLOPT_SSL_VERIFYPEER => false,        // chen_debug       // enable for localhost; if you want to be online, please remove these lines.
            CURLOPT_ENCODING => "",
            ...
            CURLOPT_HTTPHEADER => array(
                'content-type: application/json',
                "user-agent: MailChimp for Cartthrob/{$env->version}",
            )
        );
    }
