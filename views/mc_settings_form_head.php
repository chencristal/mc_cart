...
<script type="text/javascript">
	...
	jQuery.cartthrobCP = {
		currentSection: function() {
			if (window.location.hash && window.location.hash != '#') {
				...
				return $('#mc_cart_settings_content h3:first').attr('data-hash');
			}
		},
	...
	jQuery(document).ready(function($){
		...
		$("#mc_cart_settings_content").accordion({
			autoHeight: false,
			header: "h3.accordion",
		...
		$(document).on('change', '.email_event_setting_option select', function(){
			notificationSetter(this); 
		});
		...
		$(document).on('change', 'select.product_channel', function(){
		...
		$(document).on('click', 'a.remove_product_table', function(){
			if (confirm('Are you sure you want to delete this row?'))
			{
		...
		$(document).on('mouseover', 'a.remove_product_table', function(){
			$(this).find('img').animate({opacity:1});
		});
		$(document).on('mouseout', 'a.remove_product_table', function(){
			$(this).find('img').animate({opacity:.2});
		});
		$('a.remove_product_table').find('img').css({opacity:.2});
